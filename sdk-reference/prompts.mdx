...
> **Note**: Prompt names must be unique within your Quotient account. Attempting to create a prompt with an existing name will raise an error.
...
> **Tip**: The returned `Prompt` object includes a `messages` property, useful for integration with model provider libraries (e.g., OpenAI).
...
> **Note**: Prompts are soft-deleted -- if you'd like to permanently remove prompts, contact us at [support@quotientai.co](mailto:support@quotientai.co).
