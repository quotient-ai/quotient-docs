...
## Prompt Structure
Prompts can use `{{input}}` and `{{context}}` as variables to dynamically pass values during prompt execution. This allows for flexibility in testing different inputs.
