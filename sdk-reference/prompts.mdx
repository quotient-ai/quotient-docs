...
<Tip> The returned `Prompt` object includes a `messages` property, useful for integration with model provider libraries (e.g., OpenAI).</Tip>
...
<Tip>[Watch `List Prompts in Quotient SDK` on YouTube](https://youtu.be/iDSdJ-a7ouc)</Tip>
---
...
```python
updated_user_prompt = "Appropriately answer customer questions. Be concise in your answers."
prompt.user_prompt = updated_user_prompt
```

<Tip>[Watch `Update User Prompts in Quotient SDK 🤖` on YouTube](https://youtu.be/aWGyqV02sBc)</Tip>
...
<Tip>[Watch `Delete Prompts in Quotient SDK 👋` on YouTube](https://youtu.be/FIBMZevWrDo)</Tip>
