...
=======
    ...
    ```
>>>>>>> de2b776 (Remove mint.json, upgrade to docs.json. Update docs)
...
* `Log` **(object)**: A `Log` object containing the following fields:
    ...
    * `instructions` **(string)**: The instructions provided to the model.
    * `tags` **(array)**: List of tags associated with the log entry.
    ...
    * `has_inconsistency` **(boolean)**: Whether the log has an inconsistency.
    ...
    * `log_documents` **(array)**: Processed document information used in the log.
    * `log_message_history` **(array)**: Processed message history information used in the log.
    * `log_instructions` **(string)**: Processed instruction information used in the log.
>>>>>>> de2b776 (Remove mint.json, upgrade to docs.json. Update docs)
