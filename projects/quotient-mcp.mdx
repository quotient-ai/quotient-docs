---
title: "Quotient MCP"
description: "MCP server for evaluating tool calls and AI agent interactions"
icon: "server"
---

# Quotient MCP

An MCP server for evaluating tool calls and AI agent interactions. Analyzes and scores tool usage correctness in Agent contexts.

## Remote Server

Quotient MCP is available as a remote Streamable HTTP server:

```
https://mcp.quotientai.co/mcp/
```
...
The evaluation is powered by the [Limbic Tool Use Evaluator](https://huggingface.co/quotientai/limbic-tool-use-0.5B-32K), a fine-tuned Qwen2.5-0.5B model specifically designed for MCP function call evaluation.
...
Add evaluation rule in `.cursor/rules/evaluate-tool-call.mdc`:
...
# Add custom prompt prefix for tool use evaluation
...
agents = [
    Agent(
        ...
        instructions=TOOL_USE_EVAL_PROMPT_PREFIX + RECOMMENDED_PROMPT_PREFIX + instructions,
        external_connections=[quotient_mcp]
    )
]
```
